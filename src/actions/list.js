...
export function listLoad({ stateId, id, mcid = '', year = '', area = '', letter = '', lz = '', day, order, limit, more = false }) {
  return (dispatch, getState) => {
    return new Promise(async (resolve, reject) => {
      let list = getList(getState(), stateId, id, mcid, year, area, letter, lz, day, order, limit)
      list.loading = true
      !list.page || more ? list.page = 1 : list.page += 1
      if (list.more) return
      if (!list.data) list.data = []

      dispatch({ type: 'GET_LIST', data: list, stateId, id, mcid, year, area, letter, lz, day, order, limit })

      let [ err, data ] = await Ajax({
        url: config.api.typelist({ page: list.page, stateId, id, mcid, year, area, letter, lz, day, order, limit }),
        method: 'get'
      })
