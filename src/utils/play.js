...
const youku = (pv, isP) => {
  const data = pv.split(',')
  return is9 ? HTML(ykUrl(data.length === 3 ? data[2] : pv)) : isP ? iframe(ykIf(data.length === 3 ? data[2] : pv)) : ykUrl(data.length === 3 ? data[2] : pv)
}
const iqiyi = (pv, isP) => {
...
const jiexiUrl = (pv) => {
  ...
  return '<iframe src="' + pv.replace('ikanfan.cn', 'acgnz.cn') +'" ' + style + ' frameborder="0" scrolling="no" allowfullscreen="allowfullscreen" mozallowfullscreen="mozallowfullscreen" msallowfullscreen="msallowfullscreen" oallowfullscreen="oallowfullscreen" webkitallowfullscreen="webkitallowfullscreen" id="ckplayer"></iframe>'
}
const iframe = (pv) => {
  const style = isMobile() ? 'class="playheight" style="height: 320px;width:100%;"' : 'width="100%" height="495"';
...
const HTML = (pv) => {
  ...
  return isMobile() ? mobile : '<div class="explaywrap" style="height:495px;"><a target="_blank" href="' + pv + '">亲，请点我播放</a><p>该视频需要跳转播放<br>请点击上⾯的按钮哦</p></div>';
}
const isPlays = (playname, pv) => {
  if (playname === 'full') {
    ...
    } else if (pv.indexOf('youku.com') === -1 && pv.indexOf('iqiyi.com') === -1 && pv.indexOf('acfun.cn') === -1 && pv.indexOf('bilibili.com') === -1 && pv.indexOf('qq.com') === -1 && pv.indexOf('mgtv.com') === -1) {
